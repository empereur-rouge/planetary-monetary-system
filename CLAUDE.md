...
- **Routes recovery NON gated** (l'opérateur peut les utiliser pour sortir du read-only) : `/admin/compact`, `/admin/reindex-*`, `/admin/rebuild-tips`, `/admin/purge-*`, `/admin/consolidate-utxos`, `/admin/config` **GET** (lecture seule), `/admin/api-keys` CRUD, `/admin/contracts` CRUD/toggle (CF writes, pas de blocs), `/admin/gas-pool` deposit/withdraw, `/admin/rocksdb-stats`, `/admin/read-only/*`.
- **Note (v0.16.0 P2c)** : `/admin/config` **POST** est passé en `admin_writable` (gated read-only) — il ne fait plus un write CF instantané mais **forge un `GovernanceProposal`** (bloc DAG, gouvernance timelock §4). Ce n'est plus un outil de recovery (le timelock rend les desserrages non-instantanés de toute façon). Seul le GET reste recovery.
...
- **`admin_recovery`** (NON gated) — read-only sur la DB, mutations légères CF-only, ou explicitement nécessaire pour sortir du read-only : compact, reindex, rebuild-tips, purge, consolidate-utxos, config **GET** (le POST forge un bloc gouvernance → `admin_writable` depuis v0.16.0), api-keys CRUD, contracts CRUD/toggle (CF writes, pas de blocs), gas-pool deposit/withdraw, rocksdb-stats, read-only/*.
...
### Bug historique : Containers fantômes non-testnet au reboot (v0.7.13, 2026-04-28)
- **Symptôme** : Après reboot du VPS (panne réseau / OOM kernel), 4 containers (`pms-engine`, `pms-gateway`, `pms-caddy`, `pms-prometheus`) tournent avec image `:latest` (= **pre-bloom**) sur le réseau `pms-public`/`pms-internal` (sans `-testnet`), tandis que `pms-simulator-testnet` tourne sur `pms-testnet-public`. Le simulateur crash-loop avec `Cannot reach gateway at https://pms-gateway:8443/livez` parce qu'il cherche le gateway sur le mauvais réseau Docker.
- **Cause** : `/opt/pms/` contient à la fois `docker-compose.yml` (l'ancien prod) et `docker-compose.testnet.yml`. Les deux compose files utilisent le même project name (`pms`). Lors d'un précédent deploy, des containers du compose prod ont été créés (sans suffixe `-testnet`) avec leur propre `restart: unless-stopped`. Notre `upgrade-testnet.sh` crée les containers `-testnet` mais **ne supprime jamais les non-suffixés** car ils ne sont pas dans `$SERVICES_TO_RECREATE`. Au reboot, Docker auto-restarte les deux jeux, et le compose default (`docker-compose.yml`) est résolu en premier sur les noms simples → la stack stale prend le dessus.
