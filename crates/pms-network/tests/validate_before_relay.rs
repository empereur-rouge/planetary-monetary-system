...
use pms_testkit::{ephemeral_addr, spawn_node_generic_rocks, test_meta_and_wallet, wait_for_listen};
...
async fn validate_before_relay_rocks() -> Result<()> {
    ...
    let (_store_a, _dag_a, adapter_a, srv_a, _ha) =
    ...
    let (store_b, _dag_b, _adapter_b, srv_b, _hb) =
        spawn_node_generic_rocks(
            db_b.to_str().unwrap(),
            "it:vbr:B",
            b_addr.as_str(),
    ...
    let bad = WireBlock {
        ...
        signature_hex: String::new(), // signature volontairement vide → invalide
    };

    // B tente d’injecter vers A → doit être rejeté
    srv_b.broadcast(&NetMsg::Block {
        id: bad.id.clone(),
