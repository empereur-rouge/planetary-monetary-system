...
use pms_testkit::{ephemeral_addr, forge_signed_wire_block_for_test, spawn_node_generic_rocks, test_meta_and_wallet, wait_for_listen};
...
async fn inv_triggers_getblock_and_blocks_flow_rocks() -> Result<()> {
    ...
    let (a_store, _a_dag, a_adapter, a_srv, _a_jh) =
    ...
    a_srv.broadcast(&pms_network::messages::NetMsg::Block {
        id: wb.id.clone(),
