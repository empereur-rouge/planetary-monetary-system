...
async fn attacker_cannot_spend_victim_utxo_with_own_key() {
    ...
    let result = validate_transaction_full(&utxos, &theft_signed, &test_policy()).await;
...
async fn legit_owner_spend_is_accepted_bech32m() {
    ...
    let result = validate_transaction_full(&utxos, &tx_signed, &test_policy()).await;
...
async fn missing_unlocks_rejected() {
    ...
    let result = validate_transaction_full(&utxos, &tx, &test_policy()).await;
