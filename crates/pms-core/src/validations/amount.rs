...
pub fn amount_parse_pos_dec(s: &str) -> Result<Decimal, ValidationError> {
    ...
    if d <= Decimal::ZERO { return Err(ValidationError::InvalidAmount{
...
pub fn amounts_positive_outputs(outs: &[TxOutput]) -> Result<(), ValidationError> {
    for o in outs { let _ = amount_parse_pos_dec(&o.amount)?; }
    Ok(())
}
...
pub fn tx_amounts_valid(tx: &Transaction, p: &ValidatePolicy) -> Result<(), ValidationError> {
    if tx.inputs.len()  > p.max_inputs  { return Err(ValidationError::TooManyInputs); }
    if tx.outputs.len() > p.max_outputs { return Err(ValidationError::TooManyOutputs); }

    let raw = serde_json::to_vec(tx).map_err(|_| ValidationError::Other("serde tx"))?;
    if raw.len() > p.max_tx_bytes { return Err(ValidationError::TxTooLarge); }

    for o in &tx.outputs { let _ = amount_parse_pos_dec(&o.amount)?; }
...
pub fn amount_is_positive_decimal(s: &str) -> bool {
    ...
    let mut dots = 0;
    for &c in bytes {
        if c == b'.' { dots += 1; if dots > 1 { return false; } }
        else if !(b'0'..=b'9').contains(&c) { return false; }
    }
