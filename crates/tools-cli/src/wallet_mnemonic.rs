...
pub async fn action_wallet_export_mnemonic(
    ...
    let (address, pub_hex, words): (String, String, Vec<String>) = {
        ...
        let words = match w.mnemonic_words() {
            ...
            None => {
                eprintln!("{}", "Ce wallet n’a pas de mnemonic".red());
                return Ok(());
            }
        };
    ...
    for (i, ww) in words.iter().enumerate() {
        println!("{:>2}. {}", i + 1, ww.yellow());
    }
    ...
    match how {
        1 => {
            ...
            if in_docker() {
                println!("{}", "Astuce: redirige > wallet.json".bright_black());
            }
        }
        _ => {
            let def = default_export_path(&format!(
                "wallet-seed-{}.json",
                payload["address"].as_str().unwrap_or("wallet")
            ));
            let path: String = Input::with_theme(&ColorfulTheme::default())
                .with_prompt("Chemin du fichier")
                .default(def)
                .interact_text()?;
            ...
            if in_docker() && path.starts_with("/exports/") {
                println!(
                    "{} {}",
                    "📥 Sur l’hôte:".bright_black(),
                    path.replace("/exports/", "exports/")
                );
            }
        }
    }
...
pub async fn action_wallet_import_mnemonic(state: &Arc<Mutex<CliState>>, hrp: &str) -> Result<()> {
    ...
    let words_vec: Vec<String> = match mode {
        0 => {
            // fichier
            let def = if in_docker() {
                "/exports/wallet-seed.json".to_string()
            } else {
                "wallet-seed.json".to_string()
            };
            let path: String = Input::with_theme(&ColorfulTheme::default())
                .with_prompt("Chemin du fichier JSON")
                .default(def)
                .interact_text()?;
            ...
            if txt.contains("\"private_key_b64\"") && txt.contains("\"public_key_hex\"") {
                match pms_wallet::Wallet::load_from_file(&path) {
                    Ok(w) => {
                        ...
                        println!(
                            "{}",
                            "✅ Wallet importé depuis fichier wallet".green().bold()
                        );
                        return Ok(());
                    }
                    Err(e) => {
                        eprintln!("{} {}", "Import wallet JSON échoué:".red(), e);
                        return Ok(());
                    }
                }
            }
        ...
        _ => {
            ...
            let phrase: String = Input::with_theme(&ColorfulTheme::default())
                .with_prompt("Saisis les 24 mots (séparés par des espaces)")
                .interact_text()?;
    ...
    if words_vec.len() != 24 {
        eprintln!(
            "{} {}",
            "Il faut exactement 24 mots. Reçu:".red(),
            words_vec.len()
        );
        return Ok(());
    }
    ...
    let w = match pms_wallet::Wallet::from_word_list(&words_ref) {
        Ok(w) => w,
        Err(e) => {
            eprintln!("{} {}", "Mnemonic invalide:".red(), e);
            return Ok(());
        }
    };

    {
        ...
        println!(
            "{}",
            "✅ Wallet importé et ajouté à la liste".green().bold()
        );
        println!("Adresse  : {}", st.wallets[idx].get_address(&hrp).cyan());
        println!(
            "Public   : {}",
            st.wallets[idx].encoded_public_key().yellow()
        );
    }
