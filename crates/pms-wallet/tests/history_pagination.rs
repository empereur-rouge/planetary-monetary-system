...
fn mk_sb(
    ...
    let id = compute_block_id(
        &parents,
        &payload_json.as_ref().and_then(|s| serde_json::from_str(s).ok()),
        nonce,
    );
    StoredBlock {
        id,
...
async fn history_pagination_by_time_and_id_rocks() -> Result<()> {
    ...
    for i in 1..=5u64 {
        ...
        let plain = PlainPayload::Mint {
            outputs: vec![TxOutput { address: my_addr.clone(), amount: format!("{}", 100 + i) }],
        };
        let enc = EncryptedPayload::encrypt_for_plain(&plain, &[my_xpk.clone()])
            .map_err(|e| anyhow::anyhow!("{}", e))?;
        let sb = mk_sb(vec![g.id.clone()], &PayloadEnvelope::Encrypted(enc), i, &meta);
        ...
        let other = PlainPayload::Mint {
            outputs: vec![TxOutput { address: "8e1_other_addr_____".into(), amount: "7".into() }],
        };
        let enc_o = EncryptedPayload::encrypt_for_plain(&other, &[my_xpk.clone()])
            .map_err(|e| anyhow::anyhow!("{}", e))?;
        let sb_o = mk_sb(vec![g.id.clone()], &PayloadEnvelope::Encrypted(enc_o), i + 10000, &meta);
    ...
    // ----- Act & Assert -----
    // Page 1 (limit=2)
    let page1 = pms_wallet::history::history_page_for_address(
        &*store, &my_sk, &my_addr, None, 2
    ).await?;
    assert_eq!(page1.len(), 2, "page1 doit contenir 2 éléments");
    assert!(page1[0].ts_ms >= page1[1].ts_ms, "ordre décroissant attendu");
    for e in &page1 {
        assert!(matches!(e.plain, PlainPayload::Mint{..}), "filtre par adresse");
    }

    // Cursor pour la page 2
    let after = Some((page1.last().unwrap().ts_ms, page1.last().unwrap().id.clone()));
    let page2 = pms_wallet::history::history_page_for_address(
        &*store, &my_sk, &my_addr, after, 2
    ).await?;
    ...
    let after2 = Some((page2.last().unwrap().ts_ms, page2.last().unwrap().id.clone()));
    let page3 = pms_wallet::history::history_page_for_address(
        &*store, &my_sk, &my_addr, after2, 2
    ).await?;
