...
use pms_types_payload::{PlainPayload, PayloadEnvelope, EncryptedPayload};
...
fn mk_sb(id: String, parents: Vec<String>, payload: &PayloadEnvelope, nonce: u64, meta: &WireMeta,) -> StoredBlock {
    StoredBlock {
        id,
        parents,
...
async fn decrypt_and_filter_by_address_rocks() -> Result<()> {
    ...
    let sb_mint = mk_sb(mint_id, wb_mint_parents.clone(), &PayloadEnvelope::Encrypted(enc_mint), 1, &meta);
    ...
    let sb_tx = mk_sb(tx_id, wb_tx_parents, &PayloadEnvelope::Encrypted(enc_tx), 2, &meta);
    ...
    assert_eq!(dec.len(), 1, "Un seul bloc doit concerner mon adresse (le Mint)");
    match &dec[0].plain {
