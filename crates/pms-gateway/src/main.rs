...
async fn main() -> Result<()> {
    ...
    let api_routes = Router::new()
        ...
        // Node registration (pass-through to Engine)
        .route("/v1/register", post(routes::proxy_post))
        .route("/v1/nodes", get(routes::proxy_get))
        .route("/v1/peers", get(routes::proxy_get))
        .route("/v1/fee_pool", get(routes::proxy_get))
        .route("/v1/dag/tips", post(routes::proxy_post))
        // NFT routes (proxy)
        ...
        .route("/v1/wallet/restore/private-key", post(routes::proxy_post))
