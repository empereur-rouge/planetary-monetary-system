...
impl FeePolicy {
    ...
    pub fn compute_fee(&self, amount: &str) -> Result<String, FeeError> {
        let a = Amount::parse(amount, self.precision).map_err(|_| FeeError::Amount)?.0;
        let base = Amount::parse(&self.base_fee, self.precision).map_err(|_| FeeError::Amount)?.0;
        let r = Amount::parse(&self.ratio, self.precision).map_err(|_| FeeError::Amount)?.0;
